...
- Support for masked training (Lottery Ticket experiments)
"""
...
from src.jax.networks.actor import (
    GaussianActor,
...
from src.jax.training.train_state import (
    SACTrainState,
    MaskedTrainState,
...
class SACAgent:
    ...
    Example:
        >>> config = SACConfig(gamma=0.99, tau=0.005)
        >>> agent = SACAgent(
        ...     obs_dim=10, act_dim=4,
...
def _select_action_deterministic(
    ...
    mean, _ = actor_apply_fn(actor_params, obs)
    action = deterministic_action(mean)
...
def _sac_train_step(
    ...
    def critic_loss_fn(critic_params: Params) -> Tuple[jax.Array, Metrics]:
        ...
        q1, q2 = state.critic_apply_fn(
            critic_params, batch.obs, batch.actions
        )
        ...
        next_action, next_log_prob = sample_action(
            next_mean, next_log_std, target_key
        )
